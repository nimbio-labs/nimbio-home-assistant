...
- **Never log a server-controlled dict verbatim.** Event payloads are logged as named fields plus a key list, never as `payload=%s`. The API can add fields at any time, and HA users paste debug logs into public issue reports — a dict dump silently ships whatever lands in it next.
